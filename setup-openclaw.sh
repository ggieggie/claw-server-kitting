...
if command -v openclaw &>/dev/null; then
  ...
  for i in 1 2 3 4 5; do
    ...
    if echo "$GATEWAY_STATUS" | grep -qi "running\|online\|active"; then
      log "Gateway 稼働中"
      break
    fi
    if [ "$i" -eq 5 ]; then
      warn "Gateway が応答しません。後で openclaw gateway start を実行してください"
    else
      echo "  リトライ ($i/5)..."
      sleep 3
    fi
...
echo ""
echo " 📋 次のステップ:"
echo "  0. npx playwright install chromium  # ブラウザ自動化（オプション）"
